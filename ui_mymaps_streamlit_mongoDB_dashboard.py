...
# --- AgGrid compatibility helper (automatic fallback) ---
try:
    from streamlit_aggrid import AgGrid, GridOptionsBuilder, DataReturnMode, GridUpdateMode  # type: ignore
    _AGGRID_BACKEND = "streamlit_aggrid"
except Exception:
    try:
        from streamlit_aggrid_community import AgGrid, GridOptionsBuilder, DataReturnMode, GridUpdateMode  # type: ignore
        _AGGRID_BACKEND = "streamlit_aggrid_community"
    except Exception:
        try:
            from st_aggrid import AgGrid, GridOptionsBuilder, DataReturnMode, GridUpdateMode  # type: ignore
            _AGGRID_BACKEND = "st_aggrid"
        except Exception:
            _AGGRID_BACKEND = None

def _display_grid(df, **kwargs):
    import streamlit as _st
    if _AGGRID_BACKEND is not None:
        try:
            return _display_grid(df, **kwargs)
        except Exception:
            _st.warning("AgGrid failed to render, falling back to native data editor.")
            return _st.data_editor(df, use_container_width=True)
    else:
        return _st.data_editor(df, use_container_width=True)
# --- end helper ---
